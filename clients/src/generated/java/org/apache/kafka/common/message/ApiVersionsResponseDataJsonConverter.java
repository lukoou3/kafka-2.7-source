...
public class ApiVersionsResponseDataJsonConverter {
    public static ApiVersionsResponseData read(JsonNode _node, short _version) {
        ...
        if (_apiKeysNode == null) {
            ...
            ApiVersionsResponseKeyCollection _collection = new ApiVersionsResponseKeyCollection();
            _object.apiKeys = _collection;
            for (JsonNode _element : _apiKeysNode) {
                _collection.add(ApiVersionsResponseKeyJsonConverter.read(_element, _version));
            }
        }
        ...
        if (_finalizedFeaturesNode == null) {
            ...
            FinalizedFeatureKeyCollection _collection = new FinalizedFeatureKeyCollection();
    ...
    public static JsonNode write(ApiVersionsResponseData _object, short _version) {
        ...
        for (ApiVersionsResponseKey _element : _object.apiKeys) {
            _apiKeysArray.add(ApiVersionsResponseKeyJsonConverter.write(_element, _version));
        }
        ...
        if (_version >= 3) {
            if (!_object.supportedFeatures.isEmpty()) {
                ...
                for (SupportedFeatureKey _element : _object.supportedFeatures) {
                    _supportedFeaturesArray.add(SupportedFeatureKeyJsonConverter.write(_element, _version));
                }
        ...
        if (_version >= 3) {
            if (!_object.finalizedFeatures.isEmpty()) {
                ...
                for (FinalizedFeatureKey _element : _object.finalizedFeatures) {
                    _finalizedFeaturesArray.add(FinalizedFeatureKeyJsonConverter.write(_element, _version));
                }
    ...
    public static class ApiVersionsResponseKeyJsonConverter {
        public static ApiVersionsResponseKey read(JsonNode _node, short _version) {
            ApiVersionsResponseKey _object = new ApiVersionsResponseKey();
            JsonNode _apiKeyNode = _node.get("apiKey");
            if (_apiKeyNode == null) {
                throw new RuntimeException("ApiVersionsResponseKey: unable to locate field 'apiKey', which is mandatory in version " + _version);
            } else {
                _object.apiKey = MessageUtil.jsonNodeToShort(_apiKeyNode, "ApiVersionsResponseKey");
            }
            JsonNode _minVersionNode = _node.get("minVersion");
            if (_minVersionNode == null) {
                throw new RuntimeException("ApiVersionsResponseKey: unable to locate field 'minVersion', which is mandatory in version " + _version);
            } else {
                _object.minVersion = MessageUtil.jsonNodeToShort(_minVersionNode, "ApiVersionsResponseKey");
            }
            JsonNode _maxVersionNode = _node.get("maxVersion");
            if (_maxVersionNode == null) {
                throw new RuntimeException("ApiVersionsResponseKey: unable to locate field 'maxVersion', which is mandatory in version " + _version);
            } else {
                _object.maxVersion = MessageUtil.jsonNodeToShort(_maxVersionNode, "ApiVersionsResponseKey");
            }
            return _object;
        }
        public static JsonNode write(ApiVersionsResponseKey _object, short _version) {
            ObjectNode _node = new ObjectNode(JsonNodeFactory.instance);
