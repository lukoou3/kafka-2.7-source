...
import static org.apache.kafka.common.message.EndQuorumEpochRequestData.*;
...
public class EndQuorumEpochRequestDataJsonConverter {
    public static EndQuorumEpochRequestData read(JsonNode _node, short _version) {
        ...
        if (_clusterIdNode == null) {
            ...
            if (_clusterIdNode.isNull()) {
                ...
                if (!_clusterIdNode.isTextual()) {
                    throw new RuntimeException("EndQuorumEpochRequestData expected a string type, but got " + _node.getNodeType());
                }
        ...
        if (_topicsNode == null) {
            ...
            if (!_topicsNode.isArray()) {
                throw new RuntimeException("EndQuorumEpochRequestData expected a JSON array, but got " + _node.getNodeType());
            }
    ...
    public static class PartitionDataJsonConverter {
        ...
        public static JsonNode write(PartitionData _object, short _version) {
            ...
            for (Integer _element : _object.preferredSuccessors) {
                _preferredSuccessorsArray.add(new IntNode(_element));
            }
